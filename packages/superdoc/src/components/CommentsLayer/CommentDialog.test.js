...
const mountDialog = async ({
  ...
  commentsStoreStub = {
    ...
    // Story-aware accept/reject routing (Phase 6). The stub mirrors the real
    // store's return shape so CommentDialog code paths that check `ok`
    // / `success` flags behave identically in tests.
...
describe('CommentDialog.vue', () => {
  ...
  it('uses the reachable anchor Y for instant sidebar alignment when scroll is clamped', async () => {
    ...
    expect(commentsStoreStub.requestInstantSidebarAlignment).toHaveBeenCalledWith(expect.any(Number), 'tracked-change-1');
  });
  ...
  it('handles resolve and reject for tracked change comments', async () => {
    ...
    await nextTick();
    // Story-aware host-level routing (Phase 6): CommentDialog no longer calls
    // `editor.commands.acceptTrackedChangeById` directly. It delegates to
    // `commentsStore.decideTrackedChangeFromSidebar`, which resolves body vs.
    // non-body routing and commits non-body stories through mutatePart.
  ...
  it('uses default behavior when custom handler is not a function', async () => {
    ...
    // Default behavior routes through the host-level story-aware path.
    expect(commentsStoreStub.decideTrackedChangeFromSidebar).toHaveBeenCalledWith(
      expect.objectContaining({ comment: baseComment, decision: 'accept' }),
    );
    expect(baseComment.resolveComment).toHaveBeenCalled();
