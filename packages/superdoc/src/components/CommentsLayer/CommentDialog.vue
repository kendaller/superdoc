...
const handleReject = () => {
  ...
  if (props.comment.trackedChange && typeof customHandler === 'function') {
    ...
    // Story-aware routing: for body tracked changes the behavior is identical
    // to the legacy `rejectTrackedChangeById`. For non-body stories (headers,
    // footers, footnotes, endnotes) this goes through the document-api which
    // resolves the owning story runtime and commits through mutatePart.
...
const handleResolve = () => {
  ...
  if (props.comment.trackedChange && typeof customHandler === 'function') {
    ...
    if (props.comment.trackedChange) {
      // Story-aware routing (see handleReject for details).
      commentsStore.decideTrackedChangeFromSidebar({
        superdoc: proxy.$superdoc,
        comment: props.comment,
        decision: 'accept',
