...
import { toTrackedChangeRuntimeRef, type TrackedChangeRuntimeRef } from './tracked-change-runtime-ref.js';
...
/**
 * Convenience: derive the runtime ref for an address without forcing a full
 * resolver pass. Useful for DOM stamps and shared-map key construction.
 *
 * This intentionally duplicates the `tracked-change-runtime-ref.ts` helper
 * under a name that reads better inline — callers outside this module should
 * prefer importing from `tracked-change-runtime-ref.ts` directly.
 */
export function addressToRuntimeRef(address: TrackedChangeAddress): TrackedChangeRuntimeRef {
  return toTrackedChangeRuntimeRef(address);
}
