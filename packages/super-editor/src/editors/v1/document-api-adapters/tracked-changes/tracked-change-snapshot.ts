 ...
 *
 * Snapshots carry both the **public** address (story-aware, StoryLocator
 * based) and the **internal** runtime ref (storyKey + rawId) so that:
 *
 * - document-api consumers can return `address` directly without peeking
 *   at `storyKey`.
 * - sidebar / router code can look up the owning story runtime via
 *   `runtimeRef.storyKey` without re-running the full resolver.
 */
...
export interface TrackedChangeSnapshot {
  ...
  anchorKey: string;
  /** Absolute PM position range within the story editor (used for excerpts and live-edit anchoring). */
