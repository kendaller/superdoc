 ...
 * tracked-change identity always addresses the owning part (Phase 0 rule),
 * so slot enumeration would produce duplicates against parts.
 */
export function enumerateRevisionCapableStories(editor: Editor): StoryLocator[] {
  ...
  if (Array.isArray(converter.footnotes)) {
    for (const note of converter.footnotes) {
      ...
      // They are not revision-capable; skip anything that parses negative.
