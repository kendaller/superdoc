 ...
 * them without an extra location badge. Non-body labels are localized
 * English strings; consumers that need i18n can swap this helper for a
 * translation-aware variant.
 */
export function describeStoryLocation(locator: StoryLocator): string {
  switch (locator.storyType) {
    ...
    case 'headerFooterPart':
      // Without section context we can only identify the part.
      // Callers that know the owning section can override via the registry.
