...
describe("source node paths", () => {
  ...
  it("builds nested run paths from the actual paragraph tree", async () => {
    ...
    const runPaths = model.runs(firstParagraph.ref)
      .map((run) => run.sourceRefs[0]?.sourceNodePath);
  ...
  it("roots secondary-story and annotation paths to their own part containers", async () => {
    ...
    const paragraphPaths = model.allParagraphs()
      .map((paragraph) => paragraph.sourceRefs[0]?.sourceNodePath);
