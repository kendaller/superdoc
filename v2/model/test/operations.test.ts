...
import { applySemanticOperation, resetOperationCounter, SemanticHistory } from '../src/operations/index.js';
...
describe('semantic operations', () => {
  ...
  it('splits and merges a paragraph while preserving run structure', async () => {
    ...
    expect(mergedRunTexts).toEqual(['Bold', ' Red Title', ' Italic Underline']);
  });

  it('materializes the first run when insertText targets a runless paragraph', async () => {
    const { session, model } = await openSemanticModel(createMinimalDocx('Hello'));
    const history = new SemanticHistory();
    const paragraph = model.allParagraphs()[0];
    ...
    const splitResult = await applySemanticOperation(
      {
        id: 'split-empty-1',
        label: 'Split paragraph at end',
        kind: 'splitParagraph',
        target: paragraph.ref,
        at: { runIndex: 0, charOffset: 5 },
      },
      model,
      session,
    );
    ...
    const emptyParagraph = model.allParagraphs()[1];
    expect(emptyParagraph).toBeDefined();
    expect(model.runs(emptyParagraph.ref)).toHaveLength(0);

    const insertResult = await applySemanticOperation(
      {
        id: 'insert-empty-1',
        label: 'Insert into empty paragraph',
        kind: 'insertText',
        target: emptyParagraph.ref,
        text: 'World',
        position: { segmentIndex: 0, charOffset: 0 },
      },
      model,
      session,
      history,
    ...
    expect(readDocumentXml(session)).toContain('<w:p><w:r><w:t>World</w:t></w:r></w:p>');

    const updatedEmptyParagraph = model.allParagraphs()[1];
    const insertedRun = model.runs(updatedEmptyParagraph.ref)[0];
    expect(insertedRun).toBeDefined();
    expect(textSegmentsForRun(model, insertedRun.ref)).toBe('World');

    const undoOp = history.undo();
    expect(undoOp).toBeDefined();
    const undoResult = await applySemanticOperation(undoOp!, model, session);
    expect(undoResult.ok).toBe(true);
    expect(model.runs(model.allParagraphs()[1].ref)).toHaveLength(0);
  });
});
