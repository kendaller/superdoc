...
function makeEntry(id: string, label: string, docClass: "A" | "B" | "C" | "D"): CorpusEntry {
  return { id, class: docClass, label, description: "", profiles: ["text-only"], bytes: 1024, estimatedPages: 5, path: "" };
}
...
describe("formatSummaryTable()", () => {
  it("formats a non-empty table", () => {
    const artifact = createArtifact(makeSnapshot(), makeEntry("a", "Doc A", "A"), "v2-static", machine, git);
    ...
    expect(table).toContain("v2-static");
  });
