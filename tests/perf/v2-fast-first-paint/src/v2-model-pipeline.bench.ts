...
describe("v2 model pipeline benchmarks", () => {
  ...
  it("benchmarks minimal docx (class A)", async () => {
    ...
    expect(result.artifact.mode).toBe("v2-static");
  ...
  it("artifact JSON is deterministic", async () => {
    ...
    const result1 = await runModelBenchmark({ entry, source, mode: "v2-static", machine, git });
    const result2 = await runModelBenchmark({ entry, source, mode: "v2-static", machine, git });
  ...
  it("timeline metrics include expected spans", async () => {
    ...
    const result = await runModelBenchmark({ entry, source, mode: "v2-static", machine, git });
