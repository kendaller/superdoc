...
test('double-click rendered endnote to edit it through the presentation surface', async ({ superdoc, browserName }) => {
  ...
  await clickTextBoundary(superdoc.page, endnote, 'endnote', 3);
  await superdoc.page.keyboard.insertText('a');
  await superdoc.waitForStable();
  await expectActiveStoryTextToContain(superdoc.page, 'endanote edited');
  await expect(endnote).toContainText('This is a simple endanote edited');
