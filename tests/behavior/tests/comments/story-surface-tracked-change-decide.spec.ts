import { test, expect, type Locator, type Page } from '../../fixtures/superdoc.js';
...
async function expectSurfaceExcerpt(page: Page, entry: (typeof STORY_CASES)[number], visible: boolean): Promise<void> {
  const surface = getSurfaceLocator(page, entry.surface);
...
for (const entry of STORY_CASES) {
  test(`accept from sidebar resolves only the ${entry.surface} tracked change and supports undo/redo`, async ({
    ...
    await expectSurfaceExcerpt(superdoc.page, entry, true);
  ...
  test(`reject from sidebar resolves only the ${entry.surface} tracked change and supports undo/redo`, async ({
    ...
    await expect
      .poll(async () => (await listTrackChanges(superdoc.page, { in: 'all' })).total)
      .toBe(STORY_CASES.length - 1);
    await expectSurfaceExcerpt(superdoc.page, entry, false);
